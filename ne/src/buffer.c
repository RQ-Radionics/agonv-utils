/* Nano-style editor - Gap buffer operations
   Copyright 2025, L.C. Benschop, Vught, The Netherlands.
   MIT license

   Ported to ESP32-MOS (agonv-utils) 2025.
   (memcpy provided by libc.a from the toolchain)
*/

#include "edit.h"

/* memcpy is available from the cross-toolchain libc.a linked via LIBGCC_FLAGS */
...
void EDT_BufAdjustCol(void)
{
    ...
    if (targetcol == 0) targetcol = EDT.cursor_col;
    EDT.curline_pos = 0;
    while (col < targetcol) {
        if (*EDT.gap_end == '\n') return;
        if (*EDT.gap_end == '\t')
...
{
    if (EDT.curline_len < (int)(EDT.mem_end - EDT.cut_end)) {
        memcpy(EDT.cut_end, EDT.gap_end, (size_t)(EDT.curline_len + 1));
...
void EDT_BufPaste(void)
{
    if ((EDT.gap_end - EDT.gap_start) > (EDT.cut_end - EDT.text_end) &&
        EDT.cut_lines > 0) {
        memcpy(EDT.gap_start, EDT.text_end,
               (size_t)(EDT.cut_end - EDT.text_end));
