...
//UNUSED
//// Cursor up and down
//#define ESC_CURSOR_UP   "\xb"
//#define ESC_CURSOR_DOWN "\xa"
...
char *strchrnul(const char *s, int c) {
	...
	return (char *)(res ? res : s + strlen(s));
}
...
# define indentcol char_insert__indentcol
	...
	if (c == 22) {		// Is this an ctrl-V?
		p += (int)stupid_insert(p, '^');	// use ^ to indicate literal next
...
# endif
		...
		modified_count++;
#endif
		p += 1 + (int)stupid_insert(p, c);	// insert the char
#if ENABLE_FEATURE_VI_SETOPTS
		...
		do {
			...
			if (dot < end - 1) {	// make sure not last char in text[]
				...
				*dot++ = ' ';
				modified_count++;
#endif
				while (vi_isblank(*dot)) {	// delete leading WS
