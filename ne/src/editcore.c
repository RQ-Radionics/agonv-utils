/* Nano-style editor - Core editing logic
   Copyright 2025, L.C. Benschop, Vught, The Netherlands.
   MIT license

   Ported to ESP32-MOS (agonv-utils) 2025.
   System headers replaced by edit.h / mos_libc.h.
*/

#include "edit.h"

/* putch helper (already defined as macro in edit.h via VDUWrite,
   but editcore.c uses putch() directly — provide it here) */
...
{
    EDT_BufInsertChar((unsigned char)c);
    EDT.cursor_col_max = 0;
    EDT_RenderCurrentLine();
}
...
static void EDT_GotoLine(void)
{
    ...
    if (n < 1) n = 1;
    if (n > (int)EDT.total_lines) n = (int)EDT.total_lines;
    if (n < (int)EDT.lineno) {
        for (i = 0; i < (int)EDT.lineno - n; i++)
            EDT_BufPrevLine();
        EDT_BufAdjustCol();
    } else if (n > (int)EDT.lineno) {
        for (i = 0; i < n - (int)EDT.lineno; i++)
            EDT_BufNextLine();
        EDT_BufAdjustCol();
    }
    EDT.lineno     = (unsigned int)n;
    EDT.cursor_row = 127;
    EDT_AdjustTop(false);
}
...
static void EDT_SearchF(void)
{
    ...
    if (srch_len > 0) {
        while ((unsigned int)(EDT.text_end - p) >= srch_len) {
            if (memcmp(p, EDT.mem_start + SEARCHSTRING_OFFS, srch_len) == 0) {
                while (p != EDT.gap_end) EDT_BufNextChar();
                EDT_BufStartLine();
                EDT.cursor_col     = 0;
                EDT.cursor_col_max = 0;
                EDT_BufAdjustCol();
                EDT.curline_pos = (unsigned char)(p - EDT.gap_end);
                while (p != EDT.gap_end) EDT_BufNextChar();
...
static void EDT_SearchB(void)
{
    ...
    if (srch_len > 0) {
        while (p > EDT.text_start) {
            if (memcmp(p, EDT.mem_start + SEARCHSTRING_OFFS, srch_len) == 0) {
                while (p != EDT.gap_start) EDT_BufPrevChar();
                EDT_BufStartLine();
                EDT.cursor_col     = 0;
                EDT.cursor_col_max = 0;
                EDT_BufAdjustCol();
                EDT.curline_pos = (unsigned char)(p - EDT.gap_start);
                while (p != EDT.gap_start) EDT_BufNextChar();
...
{
    unsigned char d1, d2;
    d1 = (unsigned char)toupper((int)EDT_GetKey());
    d2 = (unsigned char)toupper((int)EDT_GetKey());
    if (d1 >= 'A') d1 -= 7;
    d1 -= '0';
    if (d2 >= 'A') d2 -= 7;
    d2 -= '0';
    d1 = (unsigned char)((d1 << 4) + d2);
    if (d1 >= 32 && d1 != 127)
        EDT_InsertChar(d1);
}

static unsigned int EDT_NextWordLength(void)
{
    unsigned char *p = EDT.gap_end;
    while (*p > ' ') p++;
    return (unsigned int)(p - EDT.gap_end);
}
...
static void EDT_Justify(void)
{
    ...
    while (EDT.gap_end < EDT.text_end - 2 &&
        ...
        if (state == EDT_JUSTIFY_WORD) {
            ...
            if (EDT.gap_end[0] == '\n') {
                ...
                if (cur_len > 0) {
                    if (cur_len + 1 + wlen > (unsigned int)EDT_JUSTIFY_COLS) {
                        *EDT.gap_start++ = '\n';
                        EDT.total_lines++;
                        if (!crossed_cursor) EDT.lineno++;
                        cur_len = 0;
                    } else {
                        *EDT.gap_start++ = ' ';
                        cur_len++;
    ...
    if (crossed_cursor) {
        while (EDT.gap_start != ret_p) EDT_BufPrevChar();
    } else {
        ret_p = EDT.gap_start;
    }
    ...
    EDT.cursor_col_max = 0;
    EDT_BufAdjustCol();
    EDT.curline_pos = (unsigned char)(ret_p - EDT.gap_start);
    while (ret_p != EDT.gap_start) EDT_BufNextChar();
    EDT.cursor_row = 127;
    EDT_AdjustTop(true);
}
...
static const char HelpText[] =
    ...
    "\r\n"
    "Delete:\r\n"
    "Backspace: delete to left, Ctrl-D: delete to right\r\n"
    "\r\n"
    "Cut and paste:\r\n"
    "Ctrl-K: cut current line, repeat to cut block\r\n"
    "Ctrl-C: copy current line, repeat to copy block\r\n"
    "Ctrl-U: paste cut/copied lines\r\n"
    "\r\n"
    "Find:\r\n"
    "Ctrl-W: find forward, Ctrl-Q: find backward\r\n"
    "\r\n"
    "Other:\r\n"
    "Ctrl-R: insert file, Ctrl-O: save file\r\n"
    "Ctrl-X or ESC: exit, Ctrl-J: justify paragraph\r\n"
    "Ctrl-T + two hex digits: insert special character\r\n"
    "\r\n"
...
void EDT_EditCore(void)
{
    ...
    EDT.curline_len = (unsigned char)EDT_BufLenCurLine();
    EDT.lineno      = 1;
    EDT_ShowScreen();

    for (;;) {
        int k = (int)EDT_GetKey();
        switch (k) {
        case 1:
        ...
        case 0x100 + VKEY_LEFT:
            EDT_CursorLeft(); break;
        case 3:
            EDT_CopyLine(fCutContinue); fCutContinue = true; break;
        case 4:
        ...
        case 0x100 + VKEY_RIGHT:
            EDT_CursorRight(); break;
        case 7:
            EDT_ShowHelp(); break;
        case 8:
            EDT_GotoLine(); break;
        case 9:
            EDT_InsertChar(9); break;
        case 10:
            EDT_Justify(); break;
        case 11:
            EDT_CutLine(fCutContinue); fCutContinue = true; break;
        case 12:
            EDT_CenterScreen(); break;
        case 13:
            EDT_NewLine(); break;
        case 14:
        case 0x100 + VKEY_DOWN:
            EDT_CursorDown(); break;
        case 15:
            EDT_SetCursor(0, EDT.scr_rows - 1);
            EDT_ClrEOL();
            EDT_SetCursor(0, EDT.scr_rows - 1);
